...
def most_similar(sen, voting_dict):
    ...
    for i in voting_dict.keys():
        if i != sen:
            f.append([policy_compare(sen, i, voting_dict),i])
    f.sort()
    return f[-1][1]
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    mylist = [i.strip() for i in list(f)]
    myListDemocrat = [i for i in mylist if " D " in i]
    d = create_voting_dict(mylist)
    ddemo = create_voting_dict(myListDemocrat)
    l = [[find_average_similarity(i, d.keys(), d), i] for i in ddemo.keys()]
    l.sort()
    """
    l = [policy_compare(sen, i, voting_dict) for i in sen_set]
    return sum(l)/len(l)

most_average_Democrat = 'Nelson2' # give the last name (or code that computes the last name)
...
def find_average_record(sen_set, voting_dict):
    ...
    ddd = list(ddemo.keys())
    ddd = set(ddd)
    find_average_record(ddd, ddemo)
    """
    l = len(voting_dict[list(voting_dict.keys())[0]])
    o = [0]*l
    for i in sen_set:
        for j in range(l):
            o[j] += voting_dict[i][j]
    return [y/len(sen_set) for y in o]
...
def bitter_rivals(voting_dict):
    ...
    Example: 
        ...
        True
    """
    f = [[policy_compare(i, j, voting_dict),i,j] for i in voting_dict.keys() for j in voting_dict.keys() if i != j]
    f.sort()
    return (f[0][1], f[0][2])
